...
class RestoreNet_Unet(nn.Module):
    def __init__(self, filters=32, activation='lrelu'):
        ...
        self.conv2_1 = Conv2R(filters, filters*2)
        self.conv2_2 = Conv2R(filters*2, filters*2)
        self.pool2 = MaxPooling2D()
        
        self.conv3_1 = Conv2R(filters*2, filters*4)
        self.conv3_2 = Conv2R(filters*4, filters*4)
        self.pool3 = MaxPooling2D()
        
        self.conv4_1 = Conv2R(filters*4, filters*8)
        self.conv4_2 = Conv2R(filters*8, filters*8)
        self.pool4 = MaxPooling2D()
        
        self.conv5_1 = Conv2R(filters*8, filters*16)
        self.conv5_2 = Conv2R(filters*16, filters*16)
        # self.dropout = nn.Dropout2d(0.5)
        
        self.upv6 = ConvTranspose2D(filters*16, filters*8)
        self.concat6 = Concat()
        self.conv6_1 = Conv2R(filters*16, filters*8)
        self.conv6_2 = Conv2R(filters*8, filters*8)
        
        self.upv7 = ConvTranspose2D(filters*8, filters*4)
        self.concat7 = Concat()
        self.conv7_1 = Conv2R(filters*8, filters*4)
        self.conv7_2 = Conv2R(filters*4, filters*4)
        
        self.upv8 = ConvTranspose2D(filters*4, filters*2)
        self.concat8 = Concat()
        self.conv8_1 = Conv2R(filters*4, filters*2)
        self.conv8_2 = Conv2R(filters*2, filters*2)
        
        self.upv9 = ConvTranspose2D(filters*2, filters)
        self.concat9 = Concat()
        self.conv9_1 = Conv2R(filters*2, filters)
        self.conv9_2 = Conv2R(filters, filters)
...
class KDS(nn.Module):
    ...
    def forward(self, L, ratio):
        ...
        # R_final, I_final, output = self.KinD_noDecom(R, I, ratio)
        # I_final, I_standard = self.illum_net(I, ratio)
        # output= self.restore_net(R, I)
        output = self.restore_net(L)
